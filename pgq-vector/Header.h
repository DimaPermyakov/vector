...
template <class T>
vector<T>::vector(const vector<T>& vec) : size__(vec.size__), capacity__(vec.capacity__) {
	data__ = new T[size__];
...
inline T vector<T>::at(const size_t& index) const {
	try {
		if (index >= size__)
			throw std::out_of_range("| Index was out of range!\a");
		return data__[index];
	}
...
void vector<T>::reserve() {
	...
	for (size_t i = 0; i < size__; ++i)
		temp_data[i] = data__[i];
	data__ = new T[capacity__];
	for (size_t i = 0; i < size__; ++i)
		data__[i] = temp_data[i];
	delete[] temp_data;
}
template<class T>
